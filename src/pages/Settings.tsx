...
export default function SettingsPage({ onSettingsChange, onLogout }: SettingsPageProps) {
  ...
  const handleHardResetDatabase = async () => {
    const confirm = await showConfirm(
      'آیا از پاک کردن کامل اطلاعات دیتابیس اطمینان دارید؟',
      'با تایید این گزینه تمامی فیلم‌ها، سریال‌ها، فاکتورهای فروش، موسیقی‌ها، کاربران و تنظیمات شما به طور کامل و غیرقابل بازگشت حذف خواهند شد. این عمل به منزله بازگشت به تنظیمات کارخانه است.'
    );
    if (!confirm) return;

    try {
      showToast('در حال پاکسازی دیتابیس بومی و محلی...', 'info');
      await dbService.clearAllDatabase();
      showAlert('تمامی اطلاعات پایگاه داده با موفقیت حذف شده و به تنظیمات کارخانه بازگشتید.', 'success', 'پاکسازی موفق');
      window.location.reload();
    } catch (err: any) {
      showAlert('خطا در پاکسازی دیتابیس: ' + err.message, 'error');
    }
  };
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 h-11 bg-red-50 hover:bg-red-100 dark:bg-red-950/25 dark:hover:bg-red-950/40 border border-red-200 dark:border-red-900/40 text-red-650 dark:text-red-400 text-xs font-bold rounded-xl cursor-pointer transition-colors"
                  >
                    <Trash2 className="w-4 h-4 text-red-500" />
                    <span>حذف کل اطلاعات دیتابیس</span>
                  </button>
                </div>
              </div>
