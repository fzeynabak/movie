...
export default function SalesPage() {
  ...
  const totalRevenue = React.useMemo(() => sales.reduce((sum, s) => sum + (s.salePrice - s.discount), 0), [sales]);
  const totalCost = React.useMemo(() => sales.reduce((sum, s) => sum + s.purchasePrice, 0), [sales]);
  const totalDiscount = React.useMemo(() => sales.reduce((sum, s) => sum + s.discount, 0), [sales]);
  const totalProfit = React.useMemo(() => totalRevenue - totalCost, [totalRevenue, totalCost]);
  ...
  }, [sales, searchQuery, filterStartDate, filterEndDate]);
