...
import { X, Search, ArrowUpDown, Film, HardDrive, Info, CheckCircle, HelpCircle, AlertCircle, Loader2, Tv, ChevronDown, Check, Sparkles } from 'lucide-react';
...
export const ScanPreviewModal: React.FC<ScanPreviewModalProps> = ({
  ...
  const [batchSeriesType, setBatchSeriesType] = useState<'existing' | 'new'>('existing');
  ...
  const handleApplyBatchSeries = async () => {
    ...
    for (let i = 0; i < updatedItems.length; i++) {
      ...
      if (selectedPaths.has(item.file.fullPath)) {
        ...
        // Try to match with existing series in database to avoid TMDb searches
        ...
        if (existingSeries) {
          ...
          // If TMDb is available, fetch once in the background, else keep null
          try {
            ...
            console.error('TMDb matching failed:', err);
          }
        }
        selectedIndex++;
      }
